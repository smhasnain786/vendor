  ...
  return (
    <Link to="/" className="d-flex align-items-center gap-2">
      {isDarkMode || activeSidebarBg !== 'white' ? (
        <>
          {/* <LogoWhiteIcon /> */}
          {toggleMiniSidebar ? '' : <img src={LogoWhiteText} className="d-none d-lg-block" alt='logo-text' style={{width:'175px'}} />}
        </>
      ) : (
        <>
          {/* <LogoDarkIcon /> */}
          {toggleMiniSidebar ? '' : <img src={LogoDarkText} className="d-none d-lg-block" alt='logo-text' style={{width:'175px'}} />}
        </>
      )}
    </Link>
  );
