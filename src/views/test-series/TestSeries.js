...
const TestSeries = () => {
  ...
  const CategoryList = async() => {
   ...
   const TestSeriesList = async() => {
    const result = await testSeriesGet()
    if(result.status && result.data.length>0){
  ...
  const handleCurrencyDelete = async(row) => {
    ...
    const result  = await testSeriesDelete(data)
 ...
 const dateFormatter = (cell, row) => {
  ...
  return (
    <>
                  ...
                  <div className="card-header">
                    ...
                    <div className="dropdown">
                      ...
                      >
                        ...
                        <CSVLink className="dropdown-item" data={testSeries}>
                          Export as CSV
                        </CSVLink>
                      </div>
                      <button
      ...
      <AddTestSeries testSeries={TestSeriesList} categoryList={categoryList} show={addModalShow} onHide={() => setAddModalShow(false)} />
      <UpdateTestSeries data={testSeriesUpdate} testSeries={TestSeriesList} show={modalShow} onHide={() => setModalShow(false)} />
